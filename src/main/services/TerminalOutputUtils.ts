...
export function updateTerminalOutputLines(
  ...
  const clean = stripAnsi(incomingText)
...
/**
 * Compact and clean terminal output before passing to AI context,
 * stripping repetitive progress-bar lines and trailing whitespace.
 */
export function compactTerminalOutputForContext(lines: string[], maxLines = 100): string[] {
  if (!lines || lines.length === 0) return []

  const compacted: string[] = []
  for (let i = 0; i < lines.length; i++) {
    const current = lines[i]
    if (isProgressBarLine(current)) {
      // If next line is also a progress line, skip current
      if (i + 1 < lines.length && isProgressBarLine(lines[i + 1])) {
        continue
      }
    }
